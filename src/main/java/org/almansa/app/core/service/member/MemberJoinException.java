package org.almansa.app.core.service.member;

public class MemberJoinException extends Exception{

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	public MemberJoinException() {
		super();
		// TODO Auto-generated constructor stub
	}

	public MemberJoinException(String message, Throwable cause, boolean enableSuppression, boolean writableStackTrace) {
		super(message, cause, enableSuppression, writableStackTrace);
		// TODO Auto-generated constructor stub
	}

	public MemberJoinException(String message, Throwable cause) {
		super(message, cause);
		// TODO Auto-generated constructor stub
	}

	public MemberJoinException(String message) {
		super(message);
		// TODO Auto-generated constructor stub
	}

	public MemberJoinException(Throwable cause) {
		super(cause);
		// TODO Auto-generated constructor stub
	}
	
	

}
